...
Si no lo bumpeás, los usuarios siguen viendo el archivo viejo cacheado. Versión actual al momento de escribir esto: **v1.1.114** (23-sep-2026, tarde).
...
- **`[SECURITY-AUDIT-S1]`** (re-scoped 27-jun · 23-sep) — las dos contraseñas del panel **se rotaron el 19-sep** (Alejo, 21:11 ART; verificado por los logs de auth: login y logout de las dos cuentas a las 21:15-21:16), `ADMIN_PASS_EMPLEADO` se borró (`4b88e20`, código muerto) y `.claude/commands/security-scan.md` quedó sin valores (`db37b21`). **Queda sólo** `ADMIN_PASS` en `admin.html` (público): la usa la auth de `/api/send-notification`, y sacarla del JS va junto con `[VERCEL-ENV-VARS]`. Los valores viejos siguen en la historia de git del repo público: ya no abren nada. Ver `docs/SECURITY.md` § S1.
...
- **`[S8-STORAGE-ANON]`** (23-sep, salió del repaso · 🟠) — agujero abierto (el bucket de fotos): **sólo la keyword**. Backup local hecho el 23-sep: `D:\backups\perfume-fotos-2026-09-23\`, 165/165 objetos, 8.689.933 bytes, sha256 en `manifest.json`. El SQL lo arma el PREPARADOR. Ver `docs/SECURITY.md` § S8.
