...
Si no lo bumpeás, los usuarios siguen viendo el archivo viejo cacheado. Versión actual al momento de escribir esto: **v1.1.113** (23-sep-2026, mediodía).
...
- **`[LOG-CAMBIO-LARGO]`** (23-sep, salió de `[LOG-PULIDO]`) — `.log-cambio` es `white-space: nowrap` y un `perfume_edit` de **Foto** (13 en 60 días) trae las dos rutas enteras: a 600 px mide **1.254 px** en un feed de 523, lo recorta `main.admin-main` (`overflow-x: hidden`) y **la flecha y la URL nueva no se ven**, sin puntos suspensivos; la fila sube a 96,8 px (cronológico) / 74,8 (por perfume). Preexistente. Cómo mostrarlo lo decide el DISEÑADOR.
...
- **`[LOG-CHIP-PRECIOS]`** (23-sep, salió de `[LOG-PULIDO]`) — la chip **💰 Precios** del Log filtra `price_update`, que tiene **0 eventos en 60 días**: los 43 cambios de precio reales son `perfume_edit` con la clave `Precio` y caen en Catálogo. En producción la chip no muestra nada. Semántica de chips → DISEÑADOR.
