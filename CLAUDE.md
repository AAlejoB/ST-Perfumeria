...
Si no lo bumpeás, los usuarios siguen viendo el archivo viejo cacheado. Versión actual al momento de escribir esto: **v1.1.103** (20-sep-2026).
...
- **`[SECURITY-AUDIT-S1]`** (re-scoped 27-jun) — las constantes `ADMIN_PASS` / `ADMIN_PASS_EMPLEADO` siguen en `admin.html` (público, ~L2778); el login admin **ya no las usa** (Supabase Auth) → no es login-bypass. Queda: **rotar la contraseña** (Alejo), borrar `ADMIN_PASS_EMPLEADO` (código muerto) y sacar `ADMIN_PASS` del JS cambiando la auth de `/api/send-notification` (se cruza con `[VERCEL-ENV-VARS]`). Ver `docs/SECURITY.md` § S1. *(S10, el XSS de la tab Clientes, ya está resuelto · `f457b89`.)*
- **`[S10-BIS-XSS-ESPERA-OPINIONES]`** (20-sep, salió de cerrar S10) — el mismo stored XSS que se arregló en Clientes sigue en **Lista de espera** (`renderListaEspera`, ~L8921: `item.nombre`) y **Opiniones** (~L4564-4568: `o.nombre`, `o.texto`, también en un `title`). Las dos tablas las inserta `anon` desde el sitio. Fix: `escapeHtml()` en esos renders (patrón de `f457b89`) + bump. Ver `docs/SECURITY.md` § S10.
