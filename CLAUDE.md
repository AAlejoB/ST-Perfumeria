...
- **`[SECURITY-AUDIT-S1]` + S10** (re-scoped 27-jun) — las constantes `ADMIN_PASS` / `ADMIN_PASS_EMPLEADO` siguen en `admin.html` (público, ~L2778); el login admin **ya no las usa** (Supabase Auth) → no es login-bypass. Queda: **rotar la contraseña** (Alejo), borrar `ADMIN_PASS_EMPLEADO` (código muerto) y sacar `ADMIN_PASS` del JS cambiando la auth de `/api/send-notification` (se cruza con `[VERCEL-ENV-VARS]`). **+ S10:** stored XSS por `c.nombre` sin escapar en la tab Clientes (~L3900). Ver `docs/SECURITY.md` § S1, S10.
