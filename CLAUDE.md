...
| `promos_decants` + `promos_decants_perfumes` | `[PROMO-DECANTS]` la promo de decants (1 fila, `id = 1`: `activa`, `n`, `precio_pack`, `max_packs`, `desde`, `hasta`) y los perfumes sumados o sacados a mano · `anon` sólo lee la promo prendida y vigente · la escribe `is_jefe()`; los perfumes, las dos cuentas |
...
Si no lo bumpeás, los usuarios siguen viendo el archivo viejo cacheado. Versión actual al momento de escribir esto: **v1.1.133** (26-sep-2026, `[VALOR-INV-DEPOSITO]`).
...
16. **La cuenta del pack de decants sale de UNA función** (`[PROMO-DECANTS]` / `[DECANT-WA-TOTAL]`, 24-sep-2026): `precioPackDecants` (`app.js`) la usan la pantalla del armador, el pie y el WhatsApp (hasta v1.1.130 el WhatsApp cobraba con la escalera los de precio manual). La lista del armador (`decantEnListaArmador`) y la regla de la promo (`promoDecantEntra`) también viven en `app.js`; el panel tiene una copia **SINCRO** para «entran solos» (`promoEnLista` / `promoConEscalera`, `admin.html`): si cambiás una, cambiá la otra.
...
> **Desde 26-sep-2026** (`[VALOR-INV-DEPOSITO]`, `f461ef3`) la tarjeta 💰 de Precios & Stock (`data-role="jefe"`) es el **valor total de inventario**: local + depósito, a precio de venta (la promo si la hay), sin pausados ni sets, con el desglose exacto debajo. Es ocultamiento visual, como el resto de los roles: la empleada lee los mismos precios y cantidades.
...
- **`[COMPARE-PISA-NOMBRE]`** (24-sep, salió de la N-bis · 🟢) — a 1280, el botón «Comparar» (`.compare-btn`) se cruza con el nombre de la card: en **108 de 186** cards visibles, 49 espejadas y 59 normales (78 × 3,8 a 25,6 px). **Ya pasaba igual en v1.1.130** (`2a588ea`): no lo trajo la N. Arreglarlo es mover el botón (o el nombre): es del DISEÑADOR.
- **`[NUEVO-EN-111]`** (24-sep, del PREPARADOR · 🟢) — «Nuevo» sale en los 111 de `perfumes_nuevos` (`loadPerfumesNuevos` les pone `_isNew` a todos, `app.js` ~L4629), no en los últimos 10 que dice el comentario de `markNewPerfumes`. Queda para después.
...
**Última actualización:** **Septiembre 26, 2026** — pedido urgente de Alejo, antes de lo planificado: **`[VALOR-INV-DEPOSITO]`**, una rama y un bump: SW **v1.1.133** (`f461ef3` + `f590ea8`), en producción. La tarjeta 💰 de Precios & Stock (sólo el jefe, `[OCULTAR-VALOR-INV]`) muestra el **valor total** (local + depósito) y, debajo, cuánto es del local y cuánto del depósito, exacto; el total sigue compacto. Misma cuenta que tenía para el local: precio de venta (la promo si la hay), sin pausados ni sets. La pintan `renderPrecios` y `renderDeposito`; el realtime y el resync traen también `stock_deposito` (si no, un pase depósito→local hecho en otra tablet contaba esas unidades dos veces). Verificado con fixture (a mano, 5 cambios seguidos) y con los datos reales contra una cuenta hecha por fuera del panel: coinciden al peso. `npm run contraste`: 0 fallas + 1 token pisado, 223 mediciones. Detalle en `docs/HISTORIA.md` § "Sesión 26-sep-2026". **Pendientes (mismo orden que § Pendientes, 43):** 🔴 `[VERCEL-ENV-VARS]` · 🟠 `[SECURITY-AUDIT-S1]` · `[S13-ESCRITURAS-ANON]` · `[S10-TER-XSS-COMBOS]` · 🟡 `[RESET-EXPIRES]` · `[S3-VAULT]` · `[S4-OREGON]` · `[SUPABASE-AUTH]` · `[AUTH-ES-STAFF]` · `[BCRYPT-RESTO]` · `[TELEGRAM-PANEL-401]` · `[LIGHT-MAYO-56]` · `[COMBOS-PAUSADOS-VISIBLES]` · `[ESPERA-MAS]` · `[ORDEN-COMPRA-SUGERIDA]` · 🟢 `[LOGIN-INTENTOS-CLEANUP]` · `[MEDIR-TEMP-SWEEP]` · `[RESET-TEMP-PASSWORD-MUERTA]` · `[DC-HEADER-600]` · `[DEPOSITO-HISTORIAL-UNIFICADO]` · `[DEPOSITO-TRANSFERENCIA-EVENTO]` · `[ESPERA-ERROR-CRUDO]` · `[GUIA-DESACTUALIZADA]` · `[NAV-REPITE-BARRA]` · `[COMPARE-PISA-NOMBRE]` · `[NUEVO-EN-111]` · `[COMBO-PROMO-NULL]` · `[SW-BANNER-SMART]` · `[MODAL-PRECIO-MUERTO]` · `[CACHE-CONTROL-1W]` · `[RESUMEN-23H]` · `[SATURACION-BADGES]` · `[VERDES]` · `[ALTA-NOMBRE-3-LINEAS]` · `[LAUTARO-MIMANODERECHA]` · `[CUENTAS-POR-EMPLEADA]` · `[AVISOS-PRIORIDAD]` · `[PERMISOS-TABS-JEFE]` · `[PUNTOS-DECANTS]` · `[UPLOADER-WEBP-AUTO]` · `[TIKTOK-SLIDE]` · `[SECURITY-MD-DESCRIPCIONES-EN-PRESENTE]` · `[TAP-44]` tanda 2. **Orden de trabajo:** `[VERCEL-ENV-VARS]` (Alejo) → `[S10-TER-XSS-COMBOS]` cuando Alejo lo ponga en la fila → `[DISEÑOACORTADOR-PANELADMIN]` → `[LAUTARO-MIMANODERECHA]` → `[FACILITAR-MOBILE-EN-CATALOGO]`.
