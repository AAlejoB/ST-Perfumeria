...
**Última actualización:** Mayo 18, 2026 — sesión CLS Reserve Banners (validación post Light Mode). PSI confirmó regresión Mobile CLS 1.044 / Desktop 0.905 vs Maratón 0.0xx. Diagnóstico con `preview_inspect`: 3 elementos above-the-fold sin altura reservada (trust-badges 0→240px, quiz-cta-banner 0→235px, price-banner--big 0→129px) crecen con styles.css lazy + Supabase render y empujan todo en cascada. Fix `[CLS-BANNERS-RESERVE]` mergeado en commit `a761035` · SW v1.1.64 → **v1.1.66**. **Mobile resuelto: CLS 0.025 (-97%) · score 49 → 84 (+35) · LCP 4.6 → 2.45s.** **Desktop pendiente** (CLS sigue ~0.9 · culprit distinto: `svg.search-icon` score 0.858 + sel ST grid · queda para iter 3 próxima sesión). Ver `docs/HISTORIA.md` § "Sesión 18-may-2026 · CLS Reserve Banners" para detalle de iteraciones (v1 falló · v2 funcionó) + lecciones sobre `preview_inspect` para medir alturas reales + caveat sobre PSI siguiendo og:url/canonical hacia producción. **Pendientes abiertos:** **CLS Desktop iter 3** · cache Supabase Storage (1h → 1 año en dashboard) · A11y 92 → 100 (5 contrastes) · logo @2x retina · imágenes Supabase con `?width=400` · JS-CHUNK iter 2 · BCRYPT-MIGRATION · SUPABASE-AUTH.
