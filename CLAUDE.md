...
2. 🔴 **`[VERCEL-ENV-VARS]` (nuevo 12-ago)** — Vercel **no tiene NINGUNA variable de entorno**: el backup propio, el alta de suscriptores y el envío de push están **rotos desde mayo**. Reponer `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `ADMIN_PASS`, `VAPID_*`, `CRON_SECRET`. ⚠️ **Arreglar S11 ANTES o junto** (ver #3). La service key se copia **directo Supabase→Vercel, nunca por chat**. Ver `docs/SECURITY.md` § S12.
3. 🟠 **S11 · auth "fail-open" en `/api/send-notification`** (`api/send-notification.js` L35) — compara `adminPass !== ADMIN_PASS` sin chequear que la variable exista. Hoy inofensivo (la función igual falla), pero **en cuanto se repongan las variables sin `ADMIN_PASS`, el endpoint queda abierto para que cualquiera mande push a todos los suscriptores**. Fix de una línea: `if (!ADMIN_PASS || adminPass !== ADMIN_PASS)`.
