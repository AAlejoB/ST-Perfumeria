...
Si no lo bumpeás, los usuarios siguen viendo el archivo viejo cacheado. Versión actual al momento de escribir esto: **v1.1.83**.
...
1. ✅ ~~**`[BCRYPT-MIGRATION]` / S2**~~ — **RESUELTO el 17-sep-2026** (`98b556c` + `sql/fase1.sql` y `sql/fase3.sql` corridos por Alejo). Login por RPC, bcrypt con migración perezosa, rate-limit server-side, `anon` sin ninguna policy sobre `clientes` (verificado: 0 filas por REST, antes 98). **D ✅** (migración perezosa verificada por Alejo en producción: los 92 en plano entran con su clave de siempre y quedan hasheados). Queda: borrar los 2 clientes de prueba desde el panel · `[LOGIN-INTENTOS-CLEANUP]` 🟢 · escalón 3 (Supabase Auth) · **S13** nuevo. Ver `docs/SECURITY.md` § S2 y S13.
...
5. 🟠 **`[SECURITY-AUDIT-S1]` (re-scoped 27-jun)** — las constantes están en **L2778-2779** y **el login admin YA NO las usa** (usa Supabase Auth) → no es login-bypass. Queda: borrar `ADMIN_PASS_EMPLEADO` (código muerto) + sacar `ADMIN_PASS` del JS público cambiando la auth de `/api/send-notification` (se solapa con #2 y #3). **+ S10:** stored XSS por `c.nombre` sin escapar en la tab Clientes (~L3900).
6. 🟡 **Migrar a Supabase Auth** — solo en una "semana sin grandes cambios".
