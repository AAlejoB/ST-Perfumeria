...
**Última actualización:** Mayo 20, 2026 — sesión productiva 5 commits: `[LOGIN-RETRY-SP]` Plan A login con reintento silencioso (diseñado con ClaudeChat, archivos en `RECOMENDACIONES_CLAUDECHAT/`) · `[CACHE-CONTROL-1W]` uploads admin con cache 1 semana (sinergia con Plan B) · `[A11Y-CONTRAST-5]` 5 contrastes WCAG arreglados (a11y 92 → ~100 esperado) · `[CLS-DESKTOP-ITER3]` search-icon SVG dims (atacó el shift 0.858) · `[CLS-DESKTOP-ITER4]` section.seleccion-st min-height (parcial). SW v1.1.66 → **v1.1.71** (5 bumps). **Plan B Supabase São Paulo agendado para esta noche** post-horario perfumería · Alejo decidió adelantarlo en lugar de esperar 1-2 semanas de telemetría · ver sección "⭐ SESIÓN PRIORITARIA AGENDADA" al principio del handoff en `docs/HISTORIA.md`. **CLS Desktop sigue ~0.86 sin resolver** (iter 5 pendiente · análisis profundo del culprit arriba de #destacados con Chrome DevTools Performance trace, NO con preview_inspect que da medidas erróneas si CSS no está cargado). Mobile CLS 0.025 sigue intacto desde 18-may. **Pendientes:** ⭐ **Plan B Supabase migración esta noche** · CLS Desktop iter 5 · validar telemetría Plan A · `[SW-BANNER-SMART]` · logo @2x · imágenes Supabase con `?width=400` · JS-CHUNK iter 2 · BCRYPT-MIGRATION · SUPABASE-AUTH.
