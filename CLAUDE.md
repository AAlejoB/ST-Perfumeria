...
**Última actualización:** Mayo 21, 2026 (madrugada) — **Plan B Supabase São Paulo COMPLETADO** (commit `f532525` `[PLAN-B-SWITCH]`). Migración productiva de `us-west-2` (Oregon) → `sa-east-1` (São Paulo) · latencia esperada de ~250ms → ~30-50ms (5x más rápido). 9 pasos del playbook v2 (`RECOMENDACIONES_CLAUDECHAT/Plan_B_*.md`) ejecutados: dump pre-migración (6.2 MB en `D:\backups\`) → crear proyecto nuevo (Alejo, MICRO compute) → migrar schema (28 tablas public, 88 RLS policies, 0 diff) → migrar data (38 clientes, 167 stocks, 41 favoritos · row counts match) → migrar auth.users con hashes bcrypt PRESERVADOS (chicas entran sin reset) → Edge Functions skipped (no había) → bucket perfume-fotos migrado (100/100 archivos · cacheControl 1 semana aplicado) → switch URL+key hardcoded en admin.html/js/app.js/index.html → verificación E2E (catálogo ✓ login admin ✓ realtime ✓ storage ✓). SW v1.1.71 → **v1.1.72**. **Proyecto viejo se mantiene activo 7 días** como rollback inmediato (revert = 2 min). **NO migrado / pendiente:** notificaciones Telegram (pg_net habilitada pero el worker no procesa requests, status_code vacío en `net._http_response` · próxima sesión `[FIX-TELEGRAM-PG-NET]`). 🚨 **Issues de seguridad detectados durante esta sesión** (Alejo detectó al ver "según el código" sobre SANTOMY2026 que dije): passwords del jefe + empleada **HARDCODED en `admin.html` L2766-2767** expuestas al ver source del HTML público · bot Telegram token + chat_id visibles en función SQL · pass en plano en tabla `clientes` pendiente · agendado **`[SECURITY-AUDIT-S1]`** con pattern Claude↔Claude (urgente). **Pendientes:** revisar Plan B en 7 días + bajar viejo · `[FIX-TELEGRAM-PG-NET]` · ⚠️ **`[SECURITY-AUDIT-S1]`** (CRÍTICO · passwords hardcoded públicas) · CLS Desktop iter 5 · `[SW-BANNER-SMART]` · logo @2x · imágenes Supabase con `?width=400` · JS-CHUNK iter 2 · `[BCRYPT-MIGRATION]` · SUPABASE-AUTH.
