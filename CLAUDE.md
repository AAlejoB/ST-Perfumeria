...
- **`[S10-TER-XSS-COMBOS]`** (23-sep, decidido por Alejo como 🟠) — los datos de `combos` (nombre, categoría, ítems, precio/promo, slug) llegan **sin escapar** a `innerHTML` y a `onclick` en el panel (`loadCombos`, `renderDestacados`, el top-10 de Estadísticas) y en el sitio público (`renderSets`, hoja de detalle, carrito, comparador, Vistos recientemente, Selección ST): ~19 lugares en el panel y ~30 en `js/app.js`. Sólo escriben la tabla las dos cuentas del panel (`combos_write_staff`) y hoy las 5 filas no tienen nada peligroso. Arreglo mecánico (`escapeHtml` en el panel, `escapeHTML` en el sitio, `escapeHTML(JSON.stringify(x))` sin comillas simples en los `onclick`), tanda propia con bump. El inventario con archivo:línea vive **fuera del repo** (`_correo_agentes`). Ver `docs/SECURITY.md` § S18.
