...
- **Lo que no se hizo:** `scripts/contraste.js` no suma filas de la ventana (la pre-aprobación de C era de 4 archivos); el contraste de la ventana se midió en el DOM (2 fallas heredadas → `[JUEGOS-VENTANA-PULIDO]`). `npm run contraste` sigue en 0 / 62.
...
**Estado del repo al cierre (23-sep, noche, `_l` + `_m`):** `origin/main` = `cb65ebb` (+ este commit de docs) · `fix-chicos-24`, `fix-ver-mas` y `feat-juegos-ventana` mergeadas ff · SW en producción **v1.1.120**, verificado con `curl` · `npm run contraste` 0 fallas / 62 · `medir_targets` sobre la ventana: pestañas 176,5 × 44, × 44 × 44.
