...
- **`contraste.js` en una rama aparte** (`contraste-espera-claro`, `2be9ea4`, sin mergear): 9 filas × 2 temas para la ventana, y el ganador se elige entre la regla de la clase **y la de la etiqueta `p`** — sin eso el script no veía el 1,32. Contra `main`: 0 fallas, 62 mediciones, los 18 valores iguales a la sonda. Contra `efb9cfc`: 8 fallas (título/perfume/descripción 1,32, × 2,61). Quedó aparte porque la pre-aprobación del merge de A era de 3 archivos.
...
**Estado del repo al cierre (23-sep, noche, `_j`):** `origin/main` = `4f60dca` (+ este commit de docs) · `fix-espera-claro` y `log-pulido-2` mergeadas ff · **abierta a propósito:** `contraste-espera-claro` (sólo `scripts/contraste.js`, espera el OK de Alejo) · SW en producción **v1.1.117**, verificado con `curl` · `npm run contraste` en `main`: 0 fallas / 44 (en la rama aparte: 0 / 62) · `medir_targets` 602 / 37.
