...
- 🟠 **Hallazgo fuera de alcance, no tocado:** `renderCombos` (`admin.html` L7714-7715) concatena `s.name` y `s.cat` crudos en `innerHTML`, y `s.slug` va dentro de un `onclick` — el mismo patrón que S10 / S10-bis. `combos` sólo se escribe desde el panel autenticado, así que la explotabilidad es menor que en Clientes (una cuenta de empleada podría inyectar en el panel del jefe). Leído, no explotado; `app.js` no se revisó. Keyword propuesta: `[S10-TER-XSS-COMBOS]` — **no** se sumó a Pendientes: decide Alejo.
