...
- 🔴 **En claro, los tokens del catálogo NO mandan todavía**: el bloque de `193e3dd` (8-may-2026, *"parche masivo de legibilidad en light mode"*, **56 reglas `!important`** bajo `body:not(.dark-mode)` entre L8040 y L8200) le gana a `--tinta-precio`, `--tinta-efectivo` y a `.card-brand`. Lo que se ve en claro es `#1a1a1d` (17,36), `#1b5e20` (7,87) y `#2a2622` (15,01) — legible; los tokens quedan **declarados e inertes** hasta que se decida **`[LIGHT-MAYO-56]`**. Ese bloque se escribió porque una usuaria real no podía leer el catálogo en claro: no se toca sin decisión del DISEÑADOR.
