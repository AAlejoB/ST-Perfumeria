...
- **En claro, los tokens del catálogo toman los valores de mayo** (decisión 32): `--tinta-precio: #1a1a1d` y `--tinta-efectivo: #1b5e20` son los mismos que impone el bloque de `193e3dd` (8-may-2026, *"parche masivo de legibilidad en light mode"*, **56 reglas `!important`** bajo `body:not(.dark-mode)` entre L8040 y L8200, escritas porque una usuaria real no podía leer el catálogo). **Tokens y pantalla coinciden**: lo que se ve es `#1a1a1d` (17,36), `#1b5e20` (7,87) y, en `.card-brand`, `#2a2622` (15,01). El bloque `!important` sigue vivo hasta **`[LIGHT-MAYO-56]`**, que lo convierte a tokens; hasta entonces `.card-brand` es el único donde el token (`--gris #5e564a`) y la pantalla difieren.
- `scripts/contraste.js` mide el **valor efectivo** (cascada: `!important` → especificidad → orden) y dice qué regla lo impone; un token pisado sale ⚠️ y no cuenta como falla, porque el número que se ve pasa.
...
- `<div role="navigation">`, no `<nav>` (la regla `nav { … }` la pisaría).
