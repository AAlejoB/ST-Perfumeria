...
## 🌐 Skills globales (21-sep-2026) · valen en todos los proyectos

A diferencia de los slash commands de arriba (que viven en `.claude/commands/` **de este repo**), estas cuatro son **skills** en `C:\Users\Alejo\.claude\skills\<nombre>\SKILL.md`: Claude Code las encuentra desde cualquier proyecto, las puede activar solo cuando la tarea calza con su `description`, y **no consumen tokens hasta que se usan** (a diferencia de `CLAUDE.md`, que se carga entero en cada sesión). Salieron del esquema de roles de `CLAUDE.md` § "🎭 Roles del equipo".

| Skill | Vos escribís | Claude Code hace | Sale |
|---|---|---|---|
| **`/idea`** | `/idea que en De Calle se pueda reservar por WhatsApp` (en crudo) | Ficha con `[KEYWORD]`, tamaño S/M/L, hechos baratos del repo y **a quién le toca primero**; la guarda en `docs/IDEAS.md` (más nueva arriba) y commitea sólo ese archivo, sin push. **No toca código ni Pendientes** (eso lo decide Alejo). | La idea no se pierde ni obliga a decidir en el momento |
| **`/arranque`** | `/arranque` | Ritual git seguro (para si el árbol está sucio; `-B origin/main` sólo si la rama no tiene commits propios) + lee Pendientes, IDEAS.md, último `docs: cierre sesión`, SW real, ramas/stash | Tablero de ≤15 líneas: qué espera a 🧑 Alejo / 📐 DISEÑADOR / 🧾 PREPARADOR / 🤖 Claude Code, riesgos, y una propuesta de por dónde arrancar |
| **`/enrutar`** | Pegás un mensaje o captura de ClaudeChat (o nada: toma el último) | Parte la entrada en ítems, marca la fuente (Alejo / DISEÑADOR / PREPARADOR / "cowork sin firma"), y arma **un solo bloque al PREPARADOR** con el hecho que motiva cada ítem; lo del DISEÑADOR va adentro, marcado (regla de ruteo del 22-sep, decidida por Alejo). Filtra credenciales. `/enrutar instrucciones` genera las instrucciones fijas para los dos Proyectos de ClaudeChat. | Un bloque `🧾 Para el PREPARADOR — mandale esto tal cual` listo para pegar (+ afuera, si hay, una línea `🧑 Para Alejo`) |
| **`/bases`** | `/bases` (opcional: archivos extra, `sql`) | `origin/main`, HEAD, rama/worktree, blob **y EOL** (`i/lf w/crlf`) por archivo, `CACHE_VERSION` real de `sw.js` en `main`, árbol/ramas/stash, último cierre; con `sql`, el ref de Supabase por referencia y las reglas de SQL. Sólo lectura. | Un bloque de cita para pegar al PREPARADOR, con las reglas del patch al pie |

Ensayadas el 21-sep sobre este repo (Windows · Git Bash): todos los comandos de `/bases` y `/arranque` corren tal cual. Ejemplo real de `/bases` ese día: `origin/main 59d5be8 · admin.html cc5e4aa (i/lf w/crlf) · sw.js 8475e1c · SW v1.1.106 · último cierre 4184669 (20-sep), 4 commits después`.

Complemento ya existente en las skills sincronizadas de claude.ai: `proximo-paso` (terminar toda entrega con un bloque fijo de a quién le habla / qué espera / cuándo) — `/enrutar` es la versión con destinatarios del equipo.
