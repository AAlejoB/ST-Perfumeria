...
A diferencia de los slash commands de arriba (que viven en `.claude/commands/` **de este repo**), estas cuatro son **skills** en `C:UsersAlejo.claudeskills<nombre>SKILL.md`: Claude Code las encuentra desde cualquier proyecto, las puede activar solo cuando la tarea calza con su `description`, y **no consumen tokens hasta que se usan** (a diferencia de `CLAUDE.md`, que se carga entero en cada sesión). Salieron del esquema de roles de `CLAUDE.md` § "🎭 Roles del equipo".
...
| **`/enrutar`** | Pegás un mensaje o captura de ClaudeChat (o nada: toma el último) | Parte la entrada en ítems, marca la fuente (Alejo / DISEÑADOR / PREPARADOR / "cowork sin firma"), y arma los bloques con el hecho que los motiva. Filtra credenciales. `/enrutar instrucciones` genera las instrucciones fijas para los dos Proyectos de ClaudeChat. | Bloques `📐 Para el DISEÑADOR` · `🧾 Para el PREPARADOR` · `🧑 Para Alejo` · `🤖 Mío`, listos para pegar |
